...
#!/bin/bash  
cd ../
xcodebuild -configuration Release -target "StackMob" -sdk iphoneos4.2  

xcodebuild -configuration Release -target "StackMob" -sdk iphonesimulator4.2  
...
# are called  
 FRAMEWORK_INPUT_ARM_FILES="build/$BUILD_TYPE-iphoneos/libStackMobiOS.a"  
 FRAMEWORK_INPUT_I386_FILES="build/$BUILD_TYPE-iphonesimulator/libStackMobiOS.a"  
...
# framework with no .a extension.  
 ...
 lipo -create "$FRAMEWORK_INPUT_ARM_FILES" -arch i386 "$FRAMEWORK_INPUT_I386_FILES" -o "$FRAMEWORK_DIR/Versions/Current/$FRAMEWORK_NAME"  
